...
function ListadoMiembros() {
  ...
  const [bajaUser, setBajaUser] = useState<User | null>(null);
  ...
  const handleSolicitarBaja = async () => {
    if (!bajaUser) return;
    try {
      await api.post(`/afiliaciones/baja/${bajaUser.id}`, { motivo: bajaMotivo || undefined });
      setBajaUser(null);
      setBajaMotivo('');
    } catch (err) {
      setError(getErrorMessage(err));
  ...
  const isAdminAsociacion = currentUser?.rol === 'ADMIN_ASOCIACION';
              ...
              <tr key={user.id} class="hover:bg-slate-50">
                ...
                <td class="px-4 py-2 text-right">
                  <div class="flex flex-col items-end gap-2">
                    ...
                    {isAdminAsociacion && user.estado_reg === 'APROBADO' && user.rol === 'BASICO' && (
                      <button onClick={() => { setBajaUser(user); setBajaMotivo(''); }} class="text-red-600 hover:underline font-bold">Solicitar baja</button>
                    )}
                  </div>
                </td>
              </tr>
      ...
      {bajaUser && (
        <div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div class="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            <h3 class="text-lg font-bold text-slate-800 mb-4">Solicitar baja de socio</h3>
            <p class="text-sm text-slate-600 mb-4">¿Desea solicitar la baja de <strong>{bajaUser.nombre} {bajaUser.apellido}</strong>? La solicitud quedará pendiente de aprobación de la federación.</p>
            <div class="mb-4">
              <label class="block text-sm font-medium text-slate-700 mb-1">Motivo (opcional)</label>
              <textarea
                value={bajaMotivo}
                onInput={(e: Event) => setBajaMotivo((e.target as HTMLTextAreaElement).value)}
                rows={3}
                class="w-full px-3 py-2 border border-slate-300 rounded focus:outline-none focus:ring-1 focus:ring-slate-500"
              />
            </div>
            <div class="flex gap-3">
              <button onClick={() => setBajaUser(null)} class="w-full bg-slate-100 text-slate-700 py-2 rounded-md font-medium hover:bg-slate-200 transition-colors">Cancelar</button>
              <button onClick={handleSolicitarBaja} class="w-full bg-red-600 text-white py-2 rounded-md font-medium hover:bg-red-700 transition-colors">Solicitar baja</button>
            </div>
          </div>
        </div>
      )}
